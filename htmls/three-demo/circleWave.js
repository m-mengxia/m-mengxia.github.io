...
export function createCircleWave() {
  // 创建平面几何体
  const geometry = new THREE.PlaneGeometry(2, 2)

  const texture = new THREE.TextureLoader().load('./imgs/wave.png')
  ...
  const material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
        ...
        void main() {
            vec3 col = vec3(0.0);
            col = getScanColor(vPosition, vUv * 100.0, col);
  ...
  // 创建网格并添加到场景
  const circleWave = new THREE.Mesh(geometry, material)
  circleWave.render = function () {
